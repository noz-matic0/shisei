...
out = cv2.VideoWriter('skeleton_output.mp4', fourcc, 20.0, (int(cap.get(3)), int(cap.get(4))))
...
while cap.isOpened():
    ...
    # スケルトン（骨格）のみを描画するため、背景を黒にする
    skeleton_frame = np.zeros_like(frame)  # フレームと同じサイズの黒い画像を作成
    ...
    # フレームをリサイズ
    resized_frame = cv2.resize(skeleton_frame, (resize_width, resize_height))

    # リサイズされたフレームを表示
    cv2.imshow("Skeleton Pose Estimation", resized_frame)
